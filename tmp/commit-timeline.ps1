...
$passOutput = & "C:\Users\benke\AppData\Local\Programs\ProtonPass\pass-cli.exe" item view "moo-purplecowbrands | GitHub personal access token" --format json 2>$null | ConvertFrom-Json
$githubToken = $passOutput.extra_fields[0].content.Hidden

if (-not $githubToken) {
    Write-Error "Failed to retrieve GitHub token from Proton Pass"
    exit 1
}
